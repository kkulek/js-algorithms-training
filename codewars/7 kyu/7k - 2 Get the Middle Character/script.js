...
// cases due to an error in the test cases). You do not need to test for this.This is only
...
function getMiddle(s) {
    ...
    const letterA = (s.charAt(halfLength)).toString();
    const letterB = (s.charAt(halfLength - 1)).toString();

    return s.length % 2 === 0 ? letterB.concat('', letterA) : letterA
}
